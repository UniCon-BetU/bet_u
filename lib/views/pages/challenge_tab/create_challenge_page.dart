...
class _CreateChallengePageState extends State<CreateChallengePage> {
  ...
  Future<void> _createChallenge() async {
    ...
    final challengeTags = selectedTagText;
    // ⚠️ 스웨거 기준으로 임시 기본값/매핑 (필요시 UI에 맞춰 교체)
    final challengeScope = 'PUBLIC'; // or 'PRIVATE'

    final challengeType = 'DURATION'; // 기간형
    final challengeBetAmount = '0'; // 기본값이고 나중에 돈걸때 달라짐
    final crewId = '0'; // 있으면 실제 값으로
    final creatorId = 'user_id 넣을자리'; // 있으면 실제 값으로 이거 유저 이름 어카지 나중에..
