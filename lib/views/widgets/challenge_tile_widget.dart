...
class ChallengeTileWidget extends StatelessWidget {
  ...
  const ChallengeTileWidget({super.key, required this.c, this.background, this.trailingOverride});
  ...
  Widget build(BuildContext context) {
    final Color bgColor = background ?? bg;           // ← 우선순위
    final Widget trailing = trailingOverride ??
        Icon(
          ...
          Expanded(
            child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
              ...
              Row(children: [
                ...
                Text('${c.participants}', style: const TextStyle(fontSize: 12, color: Colors.black54)),
                const SizedBox(width: 10),
                const Icon(Icons.calendar_today_rounded, size: 13, color: Colors.black54),
                const SizedBox(width: 4),
                Text('DAY ${c.day}', style: const TextStyle(fontSize: 12, color: Colors.black54)),
              ]),
            ]),
          ),
